...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self):
        ...
        if self.use_tags:
            ...
            # Assign unique tags to each instance in the dataset that have not been assigned a tag
            new_start_tag = highest_tag + 1
            for lb in labels:
                if lb.get("tags") is None:
                    N = lb["cls"].shape[0]
                    lb["tags"] = np.arange(new_start_tag, new_start_tag + N).reshape(N, 1)
                    new_start_tag += N

            """
            # OLD WAY OF CREATING TAGS, GOOD WHEN DATA HAS NO TRACK IDS
